// SG Workout — App State & Core Logic
...
// Return the active plan array based on selected program
...
// Return a display label for the active program
...
// Switch the active program, save, and re-render current screens
...
async function initApp() {
  ...
  if (remote && Object.keys(remote).length) D = {...D, ...remote};
  if (!D.programStart) {
...
async function saveAll() { localStorage.setItem('hx_d', JSON.stringify(D)); await Sync.save(D); }
...
function exportCSV() {
  ...
  dlFile('hyrox-log.csv',['date,type,dist,dur,pace,hr,cadence,effort,weight,notes',...rows].join('\n'),'text/csv');
  showToast('✅ CSV exported!');
}
function exportJSON() {
  if (!D.logs.length) { showToast('No sessions','#ff3d55'); return; }
  dlFile('hyrox-data.json', JSON.stringify(D,null,2), 'application/json');
...
function renderSettings() {
  ...
  document.getElementById('sOwner').value = GH.owner||'';
  document.getElementById('sRepo').value  = GH.repo||'';
  document.getElementById('sStart').value = D.programStart||'';
  // Highlight active program button
  ['hyrox','gym','wl'].forEach(id => {
    const btn = document.getElementById('progBtn_'+id);
    if (btn) btn.classList.toggle('prog-btn-active', D.activeProgram === id);
  });
}
async function saveSettings() {
  const t=document.getElementById('sToken').value.trim();
  const o=document.getElementById('sOwner').value.trim();
  const r=document.getElementById('sRepo').value.trim();
  const s=document.getElementById('sStart').value;
  if(t&&o&&r) saveGHConfig(t,o,r);
  if(s) D.programStart=s;
  await saveAll();
  showToast('✅ Settings saved!');
}
async function syncNow(){ showToast('Syncing…','#ff9a35'); await Sync.save(D); }
async function loadFromGH(){
  showToast('Loading from GitHub…','#ff9a35');
  const data=await ghRead();
  if(data){D={...D,...data};localStorage.setItem('hx_d',JSON.stringify(D));showToast('✅ Loaded!');renderDash();}
  else showToast('Load failed','#ff3d55');
}
